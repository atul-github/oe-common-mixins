...
module.exports = function CryptoMixin(Model) {
  ...
  if ((Model.settings.overridingMixins && !Model.settings.overridingMixins.SoftDeleteMixin) || !Model.settings.mixins.CryptoMixin) {
    ...
    Model.evRemoveObserver('after access', cryptoMixinAfterAccessHook);
  } else {
    Model.evObserve('before save', cryptoMixinBeforeSaveHook);
    Model.evObserve('after access', cryptoMixinAfterAccessHook);
  }
};
...
function encrypt(str) {
  log.debug(log.defaultContext(), 'INFO: Encryption Algorithm defined in config.json is', '\'', oeCloud.get('encryptionAlgorithm'), '\'');
  ...
  var module = oeCloud.get('encryptionAlgorithm').split('.')[0];
  var algo = oeCloud.get('encryptionAlgorithm').split('.')[1];
  var pwd = oeCloud.get('encryptionPassword');
  if (!encryption) {
  ...
  if (!encryption[module]) {
    log.warn(log.defaultContext(), 'WARNING: Encryption algorithm module', module, 'defined in config.json (e.g., "encryptionAlgorithm": "', oeCloud.get('encryptionAlgorithm'), '") is not implemented. Falling back to crypto');
    module = 'crypto';
  }
...
function decrypt(str) {
  ...
  if (!encryption[module]) {
    log.warn(log.defaultContext(), 'WARNING: Decryption algorithm module', module, 'defined in config.json (e.g., "encryptionAlgorithm": "', oeCloud.get('encryptionAlgorithm'), '") is not implemented. Falling back to crypto');
    module = 'crypto';
  }
  var encrypter = new encryption[module]({
    'algorithm': algo,
